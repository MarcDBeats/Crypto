...
st.caption(f"⚡ Code 5.3 • Strike Price & Implied Probability • Updated: {local_time.strftime('%Y-%m-%d %H:%M:%S')} CT")
...
def fetch_kalshi_market_data(ticker):
    """Fetch full market data including strike price and implied probability"""
    try:
...
for coin in COINS:
    ...
    if ticker:
        market = fetch_kalshi_market_data(ticker)
        if market:
            kalshi_market_data[coin] = market
...
for idx, coin in enumerate(COINS):
    ...
    try:
        ...
        result = {
            ...
            'Strike_Price': strike_price,
            'Strike_Price_Str': fmt_price(strike_price),
            'Implied_Probability': implied_probability,
            'Implied_Probability_Str': fmt_kalshi(implied_probability),
            'Best_Yes_Bid': best_yes_bid,
            ...
            <div class="metric-card">
                ...
                <div class="comparison-box">
                    ...
                    <div style="display: flex; justify-content: space-between;">
                        ...
                        <span class="kalshi-val">{result['Implied_Probability_Str']}</span>
                    </div>
...
if all_results:
    df_display = pd.DataFrame([{
        ...
        'Symbol': r['Symbol'],
        'Price': r['Price_Str'],
        'Strike Price': fmt_price(r['Strike_Price']) if r['Strike_Price'] > 0 else '—',
        'Model 5m': r['Prob_5m_Str'],
        'Kalshi Implied': r['Implied_Probability_Str'],
        ...
        with col:
            ...
            <div class="{card_class}">
                ...
                <div style="display: flex; justify-content: space-between; font-size: 0.8rem; opacity: 0.8;">
                    ...
                    <span>Kalshi: {bet['Implied_Probability_Str']}</span>
                </div>
                <div style="display: flex; justify-content: space-between; font-size: 0.8rem; opacity: 0.8;">
                    <span>Strike: {fmt_price(bet['Strike_Price']) if bet['Strike_Price'] > 0 else '—'}</span>
...
# --- Footer ---
st.divider()
st.caption(f"⚡ Code 5.3 • Strike Price & Implied Probability • Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
